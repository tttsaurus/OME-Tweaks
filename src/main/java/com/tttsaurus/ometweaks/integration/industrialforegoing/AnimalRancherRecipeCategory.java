...
public class AnimalRancherRecipeCategory implements IRecipeCategory<AnimalRancherRecipeWrapper>
{
    ...
    {
        return guiHelper.drawableBuilder(new ResourceLocation(Tags.MODID, "textures/gui/jei/animal_rancher_bg.png"), 0, 0, 140, 44).build();
    }

    @Override
    public IDrawable getIcon()
    {
        return guiHelper.createDrawableIngredient(new ItemStack(BlockRegistry.animalResourceHarvesterBlock));
    }
    ...
    public void setRecipe(IRecipeLayout recipeLayout, AnimalRancherRecipeWrapper recipeWrapper, IIngredients ingredients)
    {
        ...
        if (recipeWrapper.output.itemStack != null)
            guiItemStacks.init(0, false, 115, 14);

        if (recipeWrapper.output.fluidStack != null)
            guiFluidStacks.init(0, false, 99, 15, 16, 16, recipeWrapper.output.fluidStack.amount, true, null);
